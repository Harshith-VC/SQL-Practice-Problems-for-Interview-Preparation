--From the qbstats_2015_2016 table, look only at rows from the year 2016.
--Use the lg column which stores the longest completion thrown by each quarterback.
--Find the quarterback who recorded the single longest throw.
--Return the quarterback name and the value of that longest throw.
...
(
    SELECT
        qb,
        longest_throw,
        ROW_NUMBER() OVER(ORDER BY longest_throw DESC) AS rn
    FROM Data2016
)
