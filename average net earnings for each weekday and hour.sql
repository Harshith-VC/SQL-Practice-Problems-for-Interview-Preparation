--You need to compute net earnings for each order as order_total plus tip_amount minus discount_amount minus refunded_amount.
--Then group by the weekday name of customer_placed_order_datetime and the hour of that timestamp.
--Finally compute the average net earnings for each weekday and hour.
--Round the result to two decimals and return weekday, hour, and the rounded value.
...
(
    SELECT
        DATENAME(WEEKDAY, customer_placed_order_datetime) AS weekday_name,
        DATEPART(HOUR, customer_placed_order_datetime) AS order_hour,
...
SELECT
    weekday_name,
    order_hour,
    ROUND(avg_net, 2) AS avg_net_earnings
FROM Agg
ORDER BY weekday_name, order_hour;
