...
(
    SELECT
        ABS(gun_time - net_time) AS time_diff,
        'male' AS gender
    FROM marathon_male
...
SELECT
    ABS(
        (SELECT avg_diff FROM Averages WHERE gender = 'male')
        -
        (SELECT avg_diff FROM Averages WHERE gender = 'female')
    ) AS absolute_difference_between_genders;
